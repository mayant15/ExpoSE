...
(push) ; 2
; [exec]
; inhale isStrictlyEqual(x, Null())
(declare-const $t@2@01 $Snap)
(assert (= $t@2@01 $Snap.unit))
; [eval] isStrictlyEqual(x, Null())
; [eval] Null()
(assert (isStrictlyEqual<Bool> x@1@01 (as Null<JSValue>  JSValue)))
...
(set-option :timeout 0)
(push) ; 3
(assert (not (= x@1@01 (as Null<JSValue>  JSValue))))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
; (get-info :all-statistics)
(assert (= x@1@01 (as Null<JSValue>  JSValue)))
(pop) ; 2
(pop) ; 1
